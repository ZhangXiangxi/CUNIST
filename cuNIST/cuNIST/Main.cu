...
#define copyToHost(dst, count) cudaMemcpy(dst, temp, count*sizeof(float), cudaMemcpyHostToDevice);
	...
	int mainLenet(int argc, char** argv) {
		...
		if (readData(FLAGS_train_images.c_str(), FLAGS_train_labels.c_str(), &train_images[0], &train_labels[0], width, height) != train_size)
			return 2;
		if (readData(FLAGS_test_images.c_str(), FLAGS_test_labels.c_str(), &test_images[0], &test_labels[0], width, height) != test_size)
			return 3;
		...
		for (int iter = 0; iter < FLAGS_iterations; ++iter) {
			...
			context.Backpropagation(conv1, pool1, conv2, pool2,
				...
				d_dfc1, d_dfc1relu, d_gfc2, d_gfc2bias, d_dfc2, d_onevec);
...
namespace OriginalCode {
	...
	int previousTest(int argc, char** argv) {
